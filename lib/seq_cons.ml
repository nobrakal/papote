...
let ithread gm (t:thread) : unit t =
  let rec aux gm rm = function
  | Unit ->
     return ()
  | IfThenElse (i,t,e) ->
     iarith rm i.obj >>= (fun i -> if i = 0 then aux gm rm t else aux gm rm e)
  | Assign ((x,t)) ->
     let g,e = x.obj in
     iarith rm e >>= fun e ->
  ...
  | Store (x,t) ->
     ...
     aux gm (RM.add r e rm) t
  in aux gm RM.empty t

let iprogram (p:program) : unit t =
  GS.fold (fun g acc -> acc >>= fun acc -> Cell.create 0 >|= fun c -> GM.add g c acc) (globals_of_program p) (return GM.empty)
  >>= fun gm ->
